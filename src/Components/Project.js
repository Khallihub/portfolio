const Project = ({ project, style }) => {
  return (
    <div className="flex flex-col justify-center md:w-4/6 p-4 procon" id="procon">
      <div className="flex justify-between items-center">
        <h1 className="text-4xl font-mono font-medium py-6">{project.name}</h1>
      </div>
      <div className="flex">
          ...
          <div className="flex justify-between">
            <div className="flex flex-row justify-start items-center">
              {project.techStack.map((tech) => (
                <p key={tech} className="text-xs font-mono font-light">
                  {tech} <span>&nbsp;</span>
                </p>
              ))}
            </div>
